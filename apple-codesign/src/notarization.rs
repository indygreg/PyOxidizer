...
use {
    crate::{
        app_store_connect::{AppStoreConnectClient, ConnectToken, SubmissionResponse, SubmissionResponseStatus, NewSubmissionResponse},
        reader::PathType,
        AppleCodesignError,
    },
...
fn digest_hash<H: Digest, R: Read>(reader: &mut R) -> Result<(u64, Vec<u8>), AppleCodesignError> {
...
fn digest_sha256<R: Read>(reader: &mut R) -> Result<(u64, Vec<u8>), AppleCodesignError> {
    digest_hash::<sha2::Sha256, R>(reader)
}
...
impl Notarizer {
    ...
    pub fn notarize_path(
        ...
        match PathType::from_path(path)? {
            ...
            PathType::Xar => {
                self.notarize_flat_package(path, wait_limit)
            }
            PathType::Dmg => {
                self.notarize_dmg(path, wait_limit)
            }
    ...
    pub fn notarize_bundle(
        ...
        let submission = self.create_submission(&digest)?;

        self.upload_s3_and_maybe_wait(submission, ByteStreamKind::Data(zipfile), wait_limit)
    }
    ...
    pub fn notarize_dmg(
        ...
        let (_, digest) = digest_sha256(&mut std::fs::File::open(dmg_path)?)?;

        let submission = self.create_submission(&digest)?;

        self.upload_s3_and_maybe_wait(submission, ByteStreamKind::Path(dmg_path), wait_limit)
    }
    ...
    pub fn notarize_flat_package(
        ...
        let (_, digest) = digest_sha256(&mut std::fs::File::open(pkg_path)?)?;

        let submission = self.create_submission(&digest)?;

        self.upload_s3_and_maybe_wait(submission, ByteStreamKind::Path(pkg_path), wait_limit)
    }

}

impl Notarizer {
    pub fn create_submission(&self, digest: &[u8]) -> Result<NewSubmissionResponse, AppleCodesignError> {
        ...
        Ok(client.create_submission(&hex::encode(digest), "apple-codesign")?)
    }

    fn upload_s3_and_maybe_wait(
        &self,
        submission: NewSubmissionResponse,
        upload_data: ByteStreamKind<'_>,
        wait_limit: Option<Duration>,
    ) -> Result<NotarizationUpload, AppleCodesignError> {
    ...
    fn upload_s3_package<'a>(&self, submission: &NewSubmissionResponse, bytestream: ByteStreamKind<'_>) -> Result<(), AppleCodesignError> {
        let rt = tokio::runtime::Builder::new_current_thread().enable_all().build().unwrap();
        let bytestream = match bytestream {
            ByteStreamKind::Data(data) => ByteStream::from(data),
            ByteStreamKind::Path(path) => rt.block_on(ByteStream::from_path(path))?,
        };
        // upload using s3 api
        let config = rt.block_on(aws_config::from_env()
    ...
    pub fn wait_on_notarization(&self, upload_id: &str, wait_limit: Duration) -> Result<SubmissionResponse, AppleCodesignError> {
        warn!(
